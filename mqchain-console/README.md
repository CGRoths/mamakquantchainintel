...
Discovery jobs at `/mqchain/discovery/jobs` can be completed from structured JSON result rows. Creation and completion forms return structured action states so scanner-config JSON errors, results JSON errors, and staging counts render inline. External scanners can also POST worker results to `/api/mqchain/discovery/jobs/[id]/complete`; the API accepts either `results` arrays or `resultsJson`, enforces a 1 MB body cap, and returns an explicit staged-only contract. Completion archives the JSON as a source document, creates staged candidates and inferred evidence, and still requires the normal candidate review and batch commit flow before any registry row exists. Discovery job detail pages now show scanner interface metadata, result summary counts, logs, generated source-job/document archive links, discovered candidates, evidence/status rollups, and a filtered handoff link for sending pending results into review.
