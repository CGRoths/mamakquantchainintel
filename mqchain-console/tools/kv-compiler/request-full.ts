...
import { mqKvBuilds, mqLabelBatches } from "../../src/db/schema";
...
async function main() {
  ...
  const result = await db.transaction(async tx => {
    const [triggeringBatch] = await tx.select().from(mqLabelBatches).where(eq(mqLabelBatches.id, triggeringBatchId)).limit(1);
    if (!triggeringBatch || triggeringBatch.status !== "committed") throw new Error(`triggering_batch_not_committed:${triggeringBatchId}`);
    const [latestBatch] = await tx.select({ id: mqLabelBatches.id }).from(mqLabelBatches).where(eq(mqLabelBatches.status, "committed")).orderBy(desc(mqLabelBatches.id)).limit(1);
    const request = await createFullKvBuildRequest(tx, { triggeringBatchId, lastCommittedBatchId: latestBatch?.id ?? triggeringBatchId });
    const [existing] = await tx.select().from(mqKvBuilds).where(eq(mqKvBuilds.buildHash, request.buildHash)).limit(1);
    if (existing) return existing;
    const [build] = await tx.insert(mqKvBuilds).values({ buildHash: request.buildHash, dictionaryVersion: request.snapshot.dictionaryVersion, status: "pending", rowCount: request.snapshot.registryIds.length, lastCommittedBatchId: latestBatch?.id ?? triggeringBatchId, manifest: request.manifest }).returning();
    return build;
