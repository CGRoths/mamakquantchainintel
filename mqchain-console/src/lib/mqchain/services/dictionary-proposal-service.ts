...
import {
  ...
  mqGovernanceDictionaryProposals,
  mqDictEntities,
  mqDictRoles,
...
async function allocateStableId(tx: Tx, dictionaryKind: string) {
  const ranges = await tx.select().from(mqGovernanceDictionaryIdRanges)
    .where(and(eq(mqGovernanceDictionaryIdRanges.dictionaryKind, dictionaryKind), eq(mqGovernanceDictionaryIdRanges.status, "active")))
    .orderBy(asc(mqGovernanceDictionaryIdRanges.id)).for("update");
  ...
  await tx.update(mqGovernanceDictionaryIdRanges).set({ nextId: range.nextId + 1, updatedAt: new Date() }).where(eq(mqGovernanceDictionaryIdRanges.id, range.id));
  return range.nextId;
}
...
async function applyProposal(tx: Tx, proposal: typeof mqGovernanceDictionaryProposals.$inferSelect) {
  ...
  if (proposal.proposalKind === "entity") {
    ...
    await tx.insert(mqDictEntities).values({ id, entityCode: proposal.proposedCode, entityName: proposal.proposedName, entityType: text(values.entityType), categoryId: optionalNumber(values.categoryId), websiteUrl: text(values.websiteUrl), description: text(values.description) });
  ...
  if (proposal.proposalKind === "protocol") {
    ...
    await tx.insert(mqDictProtocols).values({ id, entityId, protocolCode: proposal.proposedCode, protocolName: proposal.proposedName, protocolType: text(values.protocolType), chainScope: Array.isArray(values.chainScope) ? values.chainScope.map(String) : undefined, description: text(values.description) });
    return { subjectKind: "protocol", subjectId: id };
  }
  if (proposal.proposalKind === "category") {
    const id = await allocateStableId(tx, "category");
    await tx.insert(mqDictCategories).values({ categoryId: id, categoryCode: proposal.proposedCode, categoryName: proposal.proposedName, parentCategoryId: optionalNumber(values.parentCategoryId), domainCode: text(values.domainCode), metricDomain: text(values.metricDomain), description: text(values.description) });
    return { subjectKind: "category", subjectId: id };
  }
  if (proposal.proposalKind === "role") {
    const id = await allocateStableId(tx, "role");
    await tx.insert(mqDictRoles).values({ roleId: id, roleCode: proposal.proposedCode, roleName: proposal.proposedName, categoryId: optionalNumber(values.categoryId), roleGroup: text(values.roleGroup), metricUsageDefault: text(values.metricUsageDefault), boundaryClass: text(values.boundaryClass), defaultQualityTier: optionalNumber(values.defaultQualityTier) ?? 1, defaultFlags: optionalNumber(values.defaultFlags) ?? 0, description: text(values.description) });
    await tx.insert(mqPolicyRoleApprovalRequirements).values({ roleId: id });
    return { subjectKind: "role", subjectId: id };
  }
  if (proposal.proposalKind === "tag") {
    const id = await allocateStableId(tx, "tag");
    await tx.insert(mqDictTags).values({ id, tagCode: proposal.proposedCode, tagName: proposal.proposedName, tagGroup: text(values.tagGroup), sourceId: optionalNumber(values.sourceId) });
  ...
  if (proposal.proposalKind === "alias") {
    ...
    const [alias] = await tx.insert(mqCatalogNameAliases).values({ subjectKind, subjectId, alias: proposal.proposedName, normalizedAlias: proposal.proposedName.trim().toLowerCase().replace(/[_\s]+/g, "-") }).returning({ id: mqCatalogNameAliases.id });
  ...
  if (proposal.proposalKind === "component") {
    ...
    await tx.insert(mqDictProtocolComponents).values({
...
export async function listDictionaryProposals() {
  ...
  return getDb().select().from(mqGovernanceDictionaryProposals).orderBy(sql`${mqGovernanceDictionaryProposals.createdAt} desc`, sql`${mqGovernanceDictionaryProposals.id} desc`);
}
...
export async function createDictionaryProposal(input: unknown) {
  ...
  return getDb().transaction(async tx => {
    const [proposal] = await tx.insert(mqGovernanceDictionaryProposals).values({ ...parsed, requestedBy: actor.id }).returning();
    await tx.insert(mqAuditEvents).values({ actorId: actor.id, action: "dictionary_proposal_created", targetTable: "mq_governance_dictionary_proposals", targetId: String(proposal.id), payload: { proposalKind: proposal.proposalKind, proposedCode: proposal.proposedCode, sourceJobId: proposal.sourceJobId } });
    return proposal;
  });
}
...
export async function reviewDictionaryProposal(input: unknown) {
  ...
  return getDb().transaction(async tx => {
    ...
    const [proposal] = await tx.select().from(mqGovernanceDictionaryProposals).where(eq(mqGovernanceDictionaryProposals.id, parsed.proposalId)).for("update").limit(1);
    ...
    if (parsed.action === "approve" || parsed.action === "reject") {
      ...
      const [updated] = await tx.update(mqGovernanceDictionaryProposals).set({ status, reviewedBy: actor.id, reviewNotes: parsed.reviewNotes, reviewedAt: new Date() }).where(eq(mqGovernanceDictionaryProposals.id, proposal.id)).returning();
      await tx.insert(mqAuditEvents).values({ actorId: actor.id, action: `dictionary_proposal_${status}`, targetTable: "mq_governance_dictionary_proposals", targetId: String(proposal.id), payload: { beforeStatus: proposal.status, afterStatus: status, reviewNotes: parsed.reviewNotes } });
    ...
    const [updated] = await tx.update(mqGovernanceDictionaryProposals).set({ status: "applied", reviewedBy: proposal.reviewedBy ?? actor.id, reviewNotes: parsed.reviewNotes ?? proposal.reviewNotes, appliedAt: new Date() }).where(eq(mqGovernanceDictionaryProposals.id, proposal.id)).returning();
    await tx.insert(mqAuditEvents).values({ actorId: actor.id, action: "dictionary_proposal_applied", targetTable: "mq_governance_dictionary_proposals", targetId: String(proposal.id), payload: { appliedTarget, dictionaryVersion } });
...
export async function rerunDictionaryResolution(input: unknown) {
  ...
  return getDb().transaction(async tx => {
    await tx.execute(sql`select pg_advisory_xact_lock(70424101)`);
    const conditions = [];
    if (parsed.sourceJobId) conditions.push(eq(mqWorkflowAddressCandidates.sourceJobId, parsed.sourceJobId));
    if (parsed.candidateIds?.length) conditions.push(inArray(mqWorkflowAddressCandidates.id, parsed.candidateIds));
    const candidates = await tx.select().from(mqWorkflowAddressCandidates).where(conditions.length === 1 ? conditions[0] : sql`(${conditions[0]}) or (${conditions[1]})`).for("update");
    ...
    for (const candidate of candidates) {
      ...
      await tx.update(mqWorkflowAddressCandidates).set({ ...after, updatedAt: new Date() }).where(eq(mqWorkflowAddressCandidates.id, candidate.id));
      changes.push({ candidateId: candidate.id, before, after });
    }
    await tx.insert(mqAuditEvents).values({ actorId: actor.id, action: "candidate_dictionary_resolution_rerun", targetTable: "mq_workflow_source_jobs", targetId: parsed.sourceJobId ? String(parsed.sourceJobId) : null, payload: { dictionaryVersion: snapshot.dictionaryVersion, candidateIds: candidates.map(candidate => candidate.id), changes } });
