...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await listMetricGroups(Object.fromEntries(request.nextUrl.searchParams.entries()));

    return NextResponse.json(
      buildMetricGroupListApiResponse({
        query: {
          page: result.page,
    ...
    return errorResponse(error instanceof Error ? error.message : "Metric group list request failed.", 500);
  }
}
