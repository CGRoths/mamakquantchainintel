...
import { preflightResearchIntake } from "@/lib/mqchain/origin-client/client";
import { OriginClientError } from "@/lib/mqchain/origin-client/errors";
import { SOURCE_JOB_INTAKE_API_MAX_BODY_BYTES, researchCsvPreflightSchema } from "@/lib/mqchain/validators/intake";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = researchCsvPreflightSchema.parse(await readBoundedJsonBody(request, SOURCE_JOB_INTAKE_API_MAX_BODY_BYTES));
    return NextResponse.json(await preflightResearchIntake(body));
  } catch (error) {
    ...
    return errorResponse(error instanceof Error ? error.message : "Research preflight failed.", 500);
  }
}
